...
TodoList 是一个支持中文和英文的桌面待办应用，使用 React + Tauri 提供任务管理、拖拽排序、提醒、主题、音效和成就反馈。应用数据由随桌面程序启动的 Python 后端写入本机 SQLite；不需要账号，也不会把数据同步到云端。当前稳定版本为 **v1.0.0**。
...
- AI 助手：接入火山引擎方舟多模态模型，支持文字、语音（转写或直发）、图片和文档分析；后端使用独立的 LangGraph 对话规划与提议应用工作流，创建/更新提议在可编辑批次卡中一次确认，删除提议显示目标快照；确认直接调用后端批次接口并逐项写入，支持部分成功和失败项重试，不会再发送一条聊天消息；推挤动画布局，主题自适应
